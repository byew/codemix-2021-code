...
train_df=pd.read_csv("all_shuffle.tsv", sep='\t')
...
def generate_data(random_state=42, is_pse_label=True):
    ...
    for train_index, dev_index in skf.split(X, y):
        ...
        # test_df.to_csv(DATA_DIR + "test.tsv", sep='\t')
        if is_pse_label:
            pse_dir = "data_pse_{}/".format(i)
            pse_df = pd.read_csv(pse_dir + 'train.tsv', sep='\t')

            tmp_train_df = pd.concat([tmp_train_df, pse_df], ignore_index=True, sort=False)

        tmp_train_df.to_csv(DATA_DIR + "train.tsv", sep='\t')
        tmp_dev_df.to_csv(DATA_DIR + "dev.tsv", sep='\t')
