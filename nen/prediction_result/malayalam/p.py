...
trail = pd.read_csv('malayalam.csv')
trail_d = pd.read_csv('../../../data_2021/malayalam/test.tsv', sep='\t')
...
# trail[trail['label'].isin(['not-malayalam '])] = 1
# trail[trail['label'].isin(['not-malayalam '])] = 1
trail.loc[trail['label'] ==0, 'label']='not-malayalam'
...
result.to_csv('m_1.tsv',sep='\t' , index=0)
