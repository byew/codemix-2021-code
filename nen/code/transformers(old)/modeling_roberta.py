...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense = nn.Linear(hidden_size_changed, hidden_size_changed)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(hidden_size_changed, config.num_labels)
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        sequence_output = outputs[0]
        last_cat = torch.cat(
            (sequence_output, outputs[2][-1], outputs[2][-2], outputs[2][-3]),
            2,
        )
        print("olddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
        # avg_p = torch.avg_pool1d(sequence_output, kernel_size=2)
        # max_p = torch.max_pool1d(sequence_output, kernel_size=2)
        # print(sequence_output.shape)
        # print(avg_p.shape)
        # print(max_p.shape)
        # h_conc_a = torch.cat(
        #     (sequence_output, avg_p, max_p), 2
        # )
...
# class RobertaForSequenceClassification_new(BertPreTrainedModel):
#     config_class = RobertaConfig
#     pretrained_model_archive_map = ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP
#     base_model_prefix = "roberta"
#
...
class RobertaForSequenceClassification_new(BertPreTrainedModel):
    ...
    Examples::
        ...
        print("newwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww")
        #
        # last_cat = last_cat[:, 0]
        # print(last_cat.shape)
...
class RobertaForSequenceClassification_cnn(BertPreTrainedModel):
    ...
    Examples::
        ...
        print("cnnnnnnnnnnnnnnnnnnnnnnnnnnnnn")

        out = torch.cat((out1, out2, out2, outputs[1]),1)
...
class RobertaForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaModel(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.classifier = nn.Linear(hidden_size_changed, 1)
        self.classifier = nn.Linear(hidden_size_changed, self.config.num_labels)
