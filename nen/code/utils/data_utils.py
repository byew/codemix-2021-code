...
class PairProcessor:
    def load_data(self, filename):
        datas = pd.read_csv(filename, sep='\t').values.tolist()
        # datas = pd.read_csv(filename).values.tolist()

        return datas

    def get_labels(self, label_directory):
        with open(label_directory, 'r') as outfile:
            data = json.load(outfile)
        outfile.close()
        return data

    def get_examples(self, data_dir, set_type):
        file_map = {'train': 'train.tsv',
                    'dev': 'dev.tsv',
                    'test': 'test.example.tsv'}
...
def classification_convert_example_to_feature(
    ...
    inputs = tokenizer.encode_plus(example.text_a,
                                   example.text_b,
                                   add_special_tokens=True, max_length=max_length, return_token_type_ids=True)
...
def compute_metrics(y_true, y_pred):
    ...
    result['weight_f1'] = round(weight_f1, 4)
